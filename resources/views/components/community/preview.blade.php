...
<div class="mt-3 row " >
    ...
    <div class="col-md-10 ml-5 pr-0 Post shadow pl-0 d-flex justify-content-between align-items-center rounded-lg" style="background: #fff">
        <div class="row row-flex w-100 pl-3">
            <div class=" col-md-1 pl-0 pr-0"  style="border-left: 5px solid #17756D !important; border-top-left-radius: 5px;border-bottom-left-radius:5px;background: #e0eceb; color: #707070;background-size: cover;">
                <div class="text-center ">
                    @if(auth()->user())  <a onclick="upVote({{$post->id}},{{auth()->user()->id}})">  <i   class="fa fa-caret-up fa-2x  mb-n3"  style="color: #707070" ></i></a>@endif
                    <p class=" @if(auth()->user()) mt-n2 @else mt-3 @endif" style="margin-bottom:0px!important;padding-bottom: 0px!important;"  @if(auth()->user()) id="{{$post->id}}" @endif>{{$post->upvotes_count - $post->downvotes_count}}</p>
                    @if(auth()->user()) <a onclick="downVote({{$post->id}},{{auth()->user()->id}})" > <i   class="fa fa-caret-down fa-2x mt-n3"  style="color: #707070"  ></i></a>@endif
                </div>
            ...
            <div class=" col-md-11 pt-3">
                <div class="container">
                    ...
                    <div class="d-flex align-items-center mb-2" >
                        ...
                        <a class="mr-1  font-weight-bold" style="text-decoration: none;color:#000;font-size:17px;" href="{{route('community',['id'=>$community->id])}}">{{$community->name}}</a>
                                ...
                                <div class="carousel-inner">
                                    @foreach($post->medias as $key=>$media)
                                        ...
                                        <div class="carousel-item pl-5 text-center @if($key == 0) active @endif">
                                            ...
                                            <div class="ml-2 mb-2 pl-5 nw_mg">
