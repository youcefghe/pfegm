<a href=""></a><div class="card shadow rounded-lg" style="width: 20rem;">
            ...
            @foreach ($communities as $community)
                <div class="row mb-1" style="border-bottom: 1px solid #bdbdbd" >
                    <div class="d-flex justify-content-center" >
                        <img class="mt-n1 ml-1 mb-2 shadow" src="{{$community->picture}}" style="height: 40px;width: 40px;border-radius: 50%">
                        <a class="ml-1 mt-1 text-secondary" style="text-decoration: none" href="{{route('community',['id'=>$community->id])}}"> {{$community->name}}</a>
                        <form action="{{route('Leave community',['id'=> $community->id])}}" method="DELETE">
                            @csrf
                            <button type="submit" > <i  class="fa fa-sign-out text-primary fa-lg ml-2 mt-2"  style="color: #46d160"></i></button>
                        </form>
                                            </div>
                </div>
            @endforeach
